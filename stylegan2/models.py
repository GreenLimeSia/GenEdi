    ...
    def forward(self,
        ...
        if num_latents == 1:
            ...
            if not latent_to_layer_idx:
                ...
                #     print('dlatents', dlatents[i].shape)
                dlatents = torch.cat(dlatents, dim=1)
                # print("3 " + str(dlatents.shape))
            else:
                ...
                # print("4 " + str(dlatents.shape))

        # print("5 " + str(dlatents.shape))
...
class GeneratorSynthesis(_BaseAdverserialModel):
    ...
    def __init__(self, **kwargs):
        ...
        self._num_latents = 1 + self.conv_block_size \
                            * (len(self.channels) - 1) # 1+ 2 * （9-1）=17
    ...
    def forward(self, latents):
        ...
        x = self.const.unsqueeze(0) # (1, 512, 4, 4)
        # print(x.shape)
        ...
        for block, to_data in zip(self.conv_blocks, self.to_data_layers):
            ...
            # print('yang' + str(block_latents.shape) + ' ' + str(x.shape) + str(layer_idx))
